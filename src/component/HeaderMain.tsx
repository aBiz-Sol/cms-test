...
// import { setHeaderJson } from "../../../../../redux/Slices/headerSlice";
...
const Header = () => {
  ...
  const generateHeaderData = () => {
    const headerJson = {
      ...
      getStartedMenu: {
        items: [
          { label: "Sign In", action: "toggleLogin" },
          {
            label: "Sign Up",
      ...
      userMenu: {
        items: [
          ...
          { label: "Logout", action: "toggleLogin" },
        ],
      },
      theme: {
        backgroundColor: "linear-gradient(to right, #1e3c72, #2a5298)", // Static background color
      },
    };

    // Dispatch the header JSON to Redux
    // dispatch(setHeaderJson(headerJson));
  };
                ...
                {!loggedIn && (
                  <>
                    <Button
                      ...
                      sx={{ textTransform: "none" }}
                      onClick={() => window.open("http://localhost:4040/login")}
                    >
                      Get Started
                    </Button>
                    <Menu
